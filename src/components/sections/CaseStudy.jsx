      ...
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <h2 className="font-sans font-extrabold text-4xl sm:text-5xl text-light-1 mb-4 leading-tight">
            Case Study: <span className="gradient-text">{project.title}</span>
          </h2>
          <p className="font-body text-light-4 max-w-xl text-base leading-relaxed">
            A behind-the-scenes look at my engineering process — from problem discovery to shipped product.
          </p>
        </motion.div>
        ...
        <motion.div
          ...
          className="glass rounded-2xl p-6 mb-12 grid grid-cols-2 sm:grid-cols-4 gap-6"
        >
          {metrics.map((m, i) => (
            <div key={i} className="text-center">
              <div className="font-sans font-bold text-2xl gradient-text mb-1">{m.value}</div>
              <div className="font-mono text-xs text-light-5 uppercase tracking-wider">{m.label}</div>
            </div>
          ))}
        </motion.div>
          ...
          <div className="space-y-8">
            {phases.map((phase, i) => (
              ...
              >
                ...
                <div className="glass-hover rounded-xl p-5 sm:p-6">
                  ...
                  <h3 className="font-sans font-bold text-lg text-light-1 mb-3">{phase.title}</h3>
                  <p className="font-body text-sm text-light-4 leading-relaxed">{phase.body}</p>
                </div>
              </motion.div>
            ))}
          </div>
